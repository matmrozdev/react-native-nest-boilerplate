...
- Put reusable command sequences under `maestro/subflows/` so test discovery
  does not execute them as standalone tests.
...
- Keep stable identifiers shared with the app in `packages/e2e-contract/`.
...
- inputText: ${EMAIL}
- assertVisible:
    id: ${TEST_ID_HOME_SCREEN}
```

- Name flow files in descriptive kebab-case.
- Give every top-level flow a human-readable `name` and meaningful feature and
  suite tags.
- Use `runFlow` for a repeated user interaction and pass its data through
  `env`; do not hide the main behavior under excessive subflows.
...
- Use paths relative to the calling flow so local and Maestro Cloud uploads are
  portable.
...
- Read IDs from `packages/e2e-contract`; do not create separate mobile and E2E
...
- Use visible text selectors only when the displayed copy itself is the
  behavior being verified and is intentionally stable.
...
- Use API setup to create prerequisite users or domain data when that setup is
  not the behavior under test. Use the UI when registration or another setup
  journey is itself being verified.
...
- Assert the API setup response and fail immediately when setup did not create
  the required state.

## Critical-flow coverage

Cover a user journey with Maestro when its failure would prevent a user from
reaching or completing a core product outcome. Authentication foundations must
cover registration, login, authenticated session restoration, and logout.
...
5. Change the test only after verifying that product behavior is correct and
   the test contract or assumption is wrong.

Do not mask a deterministic failure with retries or `optional: true`. Use
...
Run the static workspace validation for every Maestro or test-ID change:
...
API, and Maestro CLI are available. Report clearly when device execution was
not possible; static YAML validation is not a substitute for executing the
journey.
